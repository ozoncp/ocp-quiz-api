	...
	Context("Request", func() {
		...
		It("Create without errors", func() {
			...
			resp, err := reqApi.CreateQuiz(
				ctx, &ocpQuizApi.CreateQuizV1Request{UserId: 10, ClassroomId: 11, Link: "link"},
			)

			Expect(resp).
				To(Equal(&ocpQuizApi.CreateQuizV1Response{
					QuizId: newQuizId,
				}))
		...
		Context("MultiCreate", func() {
			request := []*ocpQuizApi.CreateQuizV1Request{
			...
			It("without errors", func() {
				...
				resp, err := reqApi.MultiCreateQuiz(
					ctx, &ocpQuizApi.MultiCreateQuizV1Request{Quizes: request},
				)

				expected := ocpQuizApi.MultiCreateQuizV1Response{QuizesIds: expIds}
				Expect(err).ToNot(HaveOccurred())
		...
		It("List without error", func() {
			...
			resp, err := reqApi.ListQuiz(
				ctx, &ocpQuizApi.ListQuizV1Request{
					Offset: offset, Limit: limit,
				},
			)

			req := make([]*ocpQuizApi.Quiz, 0, len(requests))
			for _, r := range requests {
				req = append(req, &ocpQuizApi.Quiz{
					Id:          r.Id,
					UserId:      r.UserId,
			...
			Expect(resp).
				To(Equal(&ocpQuizApi.ListQuizV1Response{
					Quizes:      req,
					CurrentPage: offset,
				}))
		...
		It("Describe existing request", func() {
			...
			resp, err := reqApi.DescribeQuiz(
				ctx, &ocpQuizApi.DescribeQuizV1Request{
					QuizId: quiz.Id,
				},
			)

			Expect(resp).
				To(Equal(&ocpQuizApi.DescribeQuizV1Response{
					Quiz: &ocpQuizApi.Quiz{
						Id:          quiz.Id,
						UserId:      quiz.UserId,
		...
		Context("Update test", func() {
			...
			update := func(expectUpdate bool) {
				...
				resp, err := reqApi.UpdateQuiz(
					ctx, &ocpQuizApi.UpdateQuizV1Request{
						QuizId:      quiz.Id,
						UserId:      quiz.UserId,
						ClassroomId: quiz.ClassroomId,
						Link:        quiz.Link,
				...
				Expect(resp).
					To(Equal(&ocpQuizApi.UpdateQuizV1Response{
			...
			It("err by repo", func() {
				...
				Expect(resp).To(Equal(&ocpQuizApi.UpdateQuizV1Response{
					Updated: false,
				}))
		...
		Context("Remove test", func() {
			remove := func(expectFound bool) {
				...
				resp, err := reqApi.RemoveQuiz(
					ctx, &ocpQuizApi.RemoveQuizV1Request{
						QuizId: requestId,
					},
				)

				Expect(resp).
					To(Equal(&ocpQuizApi.RemoveQuizV1Response{
						Found: expectFound,
					}))
		...
		Context("Invalid param", func() {
			It("ClassroomId in CreateQuizV1Request", func() {
				_, err := reqApi.CreateQuiz(
					ctx, &ocpQuizApi.CreateQuizV1Request{},
				)
			...
			It("UserId in CreateQuizV1Request", func() {
				_, err := reqApi.CreateQuiz(
					ctx, &ocpQuizApi.CreateQuizV1Request{UserId: 0, ClassroomId: 1},
				)
			...
			It("QuizId in RemoveQuizV1Request", func() {
				_, err := reqApi.RemoveQuiz(
					ctx, &ocpQuizApi.RemoveQuizV1Request{QuizId: 0},
				)
			...
			It("Limit in ListQuizV1Request", func() {
				_, err := reqApi.ListQuiz(
					ctx, &ocpQuizApi.ListQuizV1Request{Limit: 0},
				)
